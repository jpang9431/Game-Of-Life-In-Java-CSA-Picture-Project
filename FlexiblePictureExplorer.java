...
public abstract class FlexiblePictureExplorer implements MouseMotionListener, ActionListener, MouseListener {
	...
	public FlexiblePictureExplorer(DigitalPicture picture, boolean init) {
	...
	public void setImage(DigitalPicture picture) {
		...
		/*
		 * zoomFactor = 1; enableZoomItems(); hundred.setEnabled(false);
		 */
...
//	  clearInformation(); // Clear pixel information on display
		...
		scrollPane.setViewportView(imageDisplay); // Without this, the scrollPane won't update the size of the
													// imageDisplay
		repaint();
	}
	 ...
	 */
	protected void createAndInitPictureFrame() {
	...
	private void createAndInitScrollingImage() {
		...
		// imageDisplay.setToolTipText("Click a mouse button on a pixel to see the pixel
		// information");
...
//    createInfoPanel();
	...
	private void setUpNextAndPreviousButtons() {
		...
		/*
		 * Icon prevIcon = new
		 * ImageIcon(DigitalPicture.class.getResource("leftArrow.gif"),
		 * "previous index"); Icon nextIcon = new
		 * ImageIcon(DigitalPicture.class.getResource("rightArrow.gif"), "next index");
		 * // create the arrow buttons colPrevButton = new JButton(prevIcon);
		 * colNextButton = new JButton(nextIcon); rowPrevButton = new JButton(prevIcon);
		 ...
		 * 
		 * // set the sizes of the buttons int prevWidth = prevIcon.getIconWidth() + 2;
		 * int nextWidth = nextIcon.getIconWidth() + 2; int prevHeight =
		 * prevIcon.getIconHeight() + 2; int nextHeight = nextIcon.getIconHeight() + 2;
		 * Dimension prevDimension = new Dimension(prevWidth,prevHeight); Dimension
		 * nextDimension = new Dimension(nextWidth, nextHeight);
		 ...
		 * 
		 * // handle previous column button press colPrevButton.addActionListener(new
		 * ActionListener() { public void actionPerformed(ActionEvent evt) { colIndex--;
		 * if (colIndex < 0) colIndex = 0; displayPixelInformation(colIndex,rowIndex); }
		 * });
		 * 
		 * // handle previous row button press rowPrevButton.addActionListener(new
		 * ActionListener() { public void actionPerformed(ActionEvent evt) { rowIndex--;
		 * if (rowIndex < 0) rowIndex = 0; displayPixelInformation(colIndex,rowIndex); }
		 * });
		 * 
		 * // handle next column button press colNextButton.addActionListener(new
		 * ActionListener() { public void actionPerformed(ActionEvent evt) { colIndex++;
		 * if (colIndex >= picture.getWidth()) colIndex = picture.getWidth() - 1;
		 * displayPixelInformation(colIndex,rowIndex); } });
		 * 
		 * // handle next row button press rowNextButton.addActionListener(new
		 * ActionListener() { public void actionPerformed(ActionEvent evt) { rowIndex++;
	 ...
	 */
	/*
	 * private void clearInformation() { colValue.setText("N/A");
	 * rowValue.setText("N/A"); rValue.setText("R: N/A"); gValue.setText("G: N/A");
	 * bValue.setText("B: N/A"); colorPanel.setBackground(Color.black); colIndex =
	 * -1; rowIndex = -1; }
	 * 
	...
	/**
	 * Display a pop-up window with a certain String as a message that closes when
	 * clicked
	 * 
	...
	/**
	 * Test Main. It will explore the beach Note that to get this to work you will
	 * need to CHANGE THE FILE PATH for the two images
	 * 
